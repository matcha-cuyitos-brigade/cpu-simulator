...
def run_ui():
    if request.method == 'POST':  # this block is only entered when the form is submitted
        code_selected = request.form.get('code_selected')
    ...
    run_json = {
        ...
        "RAM14": my_soc.ram.ram_array[14],
        "RAM15": my_soc.ram.ram_array[15],
        "input1": my_soc.cpu.alu.input_a,
        "input2": my_soc.cpu.alu.input_b,
        "output": my_soc.cpu.alu.output,
        "flag_o": my_soc.cpu.alu.flag_overflow,
        "flag_z": my_soc.cpu.alu.flag_zero,
        "flag_n": my_soc.cpu.alu.flag_negative,
        "ir": my_soc.cpu.cu.instruction_register,
        "ira": my_soc.cpu.cu.instruction_address_register
    }

    x = json.dumps(run_json)
    return x
